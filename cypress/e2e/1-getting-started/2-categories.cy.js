...
describe("Categories", () => {
  ...
  beforeEach(() => {
    ...
    cy.get('[data-testid="categories-list"]', { timeout: 35000 });
  });
  ...
  it("Selecting a new category should deselect the old one", () => {
    ...
    cy.get('[data-testid="categories-list"]').within(() => {
      ...
      cy.get('[data-selected="false"]').should(
        "match",
        '[data-elementid="electronics"]'
      );
      cy.get('[data-selected="false"]').should(
        "match",
        `[data-elementid="men's clothing"]`
      );
      cy.get('[data-selected="false"]').should(
        "match",
        `[data-elementid="women's clothing"]`
      );
    });
  });
});
